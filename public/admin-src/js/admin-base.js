(function($) {
    ...
    $.fn.newTypeahead = function(param) {
        ...
        $(document).on('mouseout', '.dropdown-menu-search-result', function(e) {
            e.preventDefault();
            $('.dropdown-item-search-result').removeClass('active');
        });
        $(document).on('mouseenter', '.dropdown-menu-search-result', function(e) {
            e.preventDefault();
        });
        $(document).on('mousedown', '.dropdown-menu-search-result', function(e) {
            e.preventDefault();
        });
        $(document).on('mouseenter', '.dropdown-item-search-result', function(e) {
            e.preventDefault();
        });
        $(document).on('mousedown', '.dropdown-item-search-result', function(e) {
            e.preventDefault();
        });
    ...
    $.fn.alertCloseAnimation = function() {
        var $this = this;

        // set alert height
        if ($($this)[0]) {
            $($this).each(function() {
                var alertHeight = $this.parents('.alert').first().outerHeight();
                $this.parents('.alert').first().attr('style', 'height: ' + alertHeight + 'px;');
            })
        }
        ...
        $($this).on('click', function(e) {
            ...
            var alertHeight = $this.parents('.alert').first().outerHeight();
            $this.parents('.alert').first().attr('style', 'height: ' + alertHeight + 'px;');
            $this.parents('.alert').first().addClass('closing');
            setTimeout(function() {
